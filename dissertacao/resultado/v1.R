####################################
# RESULTADOS MESTRADO - MAR�O 2022 #
####################################
...
# CHAMANDO A TABELA #
data1 <- read.delim2("C:/Users/aless/Desktop/Renata/dados_mestrado_renata_completo.txt", stringsAsFactors=TRUE)
...
#   |PERGUNTA: AS LAGOAS S�O DIFERENTES EM QUANTIDADE (DOC) E QUALIDADE? (A440, SUVA, A 254, S275 E SR)  |
#   -----------------------------------------------------------------------------------------------------
...
#     -------------------------------------------------------------------------------------------------------
#    | PERGUNTA: QUAIS S�O AS CARACTER�STICAS QUE MAIS INFLUENCIAM A QUANTIDADE DE CARBONO NESSES LAGOS ?   |
#    --------------------------------------------------------------------------------------------------------
...
data1 <- read.delim("C:/Users/Renata/Google Drive/Mestrado RENATA/R/dados_mestrado_renata_completo.txt", stringsAsFactors=TRUE,dec = ",")
data1
#mydata <- na.omit(tabela_pca) # retira as linhas onde eu n�o tenho dados
View(data1)
summary(data1)
...
names(mydata.PCA)
colnames(mydata.PCA)<- c("Lago","Estação","Ano","Status","Entorno","LA","DA","Vol","DA/VOL","DR" ,"DP", "Inclinação","L", "Prof. Med","Prof. Max", "%NF","%AA","Pluviosidade",
...
#PAR�METROS A SER UTILIZADOS: volume (8), DA/vOL (9), DR (10), DP (11), inclina��o(12), prof. m�d (14), %nf(16)
...
#extrair % de explica��o das componentes
...
#   | Escolhendo as melhores vari�veis  |
#   ------------------------------------

names (data1)
## escolhendo as vari�veis de acordo com o AIC##

#names(TABELA)[X] => no colchete, colocar a vari�vel resposta
#names(TABELA)[c(11,17,14,20)] => quais as provaveis vari�veis. pode colocar quantas quiser
#intercept = TRUE, level = 1, minsize = 1, maxsize =4 ( AT� QUANTAS COMBINA��ES PARA O TAMANHO DO MODELO)
#METHOD H= RODA / D = TESTA

# -------------------------------------
# DOC ~ vari�veis de interesse :  10-da.la, 12-slope, 18-rainfall, 19-temp,14- dmean, 
#11-dp, 8-vol, 22-kd, 16-nf
# ------------------------------------
glmulti( names(data1)[29], data = data1,
...
# -------------------------------------
# SUVA ~ vari�veis de interesse :  10-da.la, 12-slope, 18-rainfall, 19-temp,14- dmean, 
#11-dp, 8-vol, 22-kd, 16-nf
# ------------------------------------

glmulti( names(data1)[34], data = data1,
...
# -------------------------------------
# S275 ~ vari�veis de interesse :  10-da.la, 12-slope, 18-rainfall, 19-temp,14- dmean, 
#11-dp, 8-vol, 22-kd, 16-nf
# ------------------------------------

glmulti( names(data1)[35], data = data1,
         names(data1)[c(8,10,11,12,14,16,18,19,22,25)], intercept = TRUE, level = 1, minsize = 1, maxsize =5,
         method = "h", crit = "aic", confsetsize = 10)
...
glmulti( names(data1)[35], data = data1,
         ...
         method = "h", crit = "aic", confsetsize = 10)
# -------------------------------------
# A254 ~ vari�veis de interesse :  10-da.la, 12-slope, 18-rainfall, 19-temp,14- dmean, 
#11-dp, 8-vol, 22-kd, 16-nf
# ------------------------------------

glmulti( names(data1)[31], data = data1,
         names(data1)[c(8,10,11,12,14,16,18,19,22,25)], intercept = TRUE, level = 1, minsize = 1, maxsize =5,
         method = "h", crit = "aic", confsetsize = 10)
...
glmulti( names(data1)[31], data = data1,
         ...
         method = "h", crit = "aic", confsetsize = 10)
# -------------------------------------
# SR ~ vari�veis de interesse :  10-da.la, 12-slope, 18-rainfall, 19-temp,14- dmean, 
#11-dp, 8-vol, 22-kd, 16-nf
# ------------------------------------

glmulti( names(data1)[33], data = data1,
         names(data1)[c(8,10,11,12,14,16,18,19,22,25)], intercept = TRUE, level = 1, minsize = 1, maxsize =5,
         method = "h", crit = "aic", confsetsize = 10)
...
names(data1)
cor(data1[,c(8,11,12,22)]) # NA NO VOLUME, POR ISSO N�O APARECE
MG.A254 <- lmer(a254~vol+dp+slope+kd+(1|lake), data=data1)
summary(MG.A254)
simulateResiduals(MG.A254, n=500, plot=T)  # ACHOU TEND�NCIA
car::Anova(MG.A254)
